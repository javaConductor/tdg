...
case class RvrFieldSetData(fields: Set[String], rvrList: List[RvrData])
{
    ...
    def purge(): RvrFieldSetData =
    {
        ...
        val (complete, incomplete) = rvrList partition
          {rvrData: RvrData => rvrData.complete}
...
case class SharedDistData(data: List[RvrFieldSetData])
{
    ...
    {
        val rfsd: Option[RvrFieldSetData] = data.find
                                            {rr: RvrFieldSetData => rvr.fieldNames.equals(rr.fields)};
        rfsd match
        {
