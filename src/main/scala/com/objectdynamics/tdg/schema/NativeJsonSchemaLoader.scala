...
import model.FieldGenConstraints
import scala.None
;
...
class NativeJsonSchemaLoader extends MetadataLoader with LogContributor
{
    ...
    private def createDataSetSpec(jsonDss: Map[String, Any]): IDataSetSpec =
    {
        ...
        return new DataSetSpec(name, fields, relationships)
    }
    ...
    private def createRelationships(dssName: String, jsonFldList: List[Any], acc: List[Relationship]): List[Relationship] =
    {
        jsonFldList match
        {
            ...
            case jsonFld :: otherFlds =>
            {
                ...
                {
                    case Some(fmap: Map[String, Any]) => fmap map
                      {(fromTo: Tuple2[String, Any]) => (fromTo._1, fromTo._2.toString)} toMap;
    ...
    {
        case dl: List[Any] => val l: List[String] = dl map
          {x: Any => x.toString};
        Some(l);
    };
    ...
    private def convertAnyToDouble(pcts: List[Any]): List[Double] =
    {
        ...
        pcts map
          {pct: Any => new BigDecimal(pct.toString).doubleValue}
    }
