...
class ParseParms(val help: String)
{
    ...
    {
        parms += name -> (default, defRex(default), false);
        cache = Some(name)
        this
    }
    ...
    {
        parms += name -> (default, defRex(default), req);
        cache = Some(name)
        this
    }

    def req(value: Boolean) =
    {
    ...
    {
        // checks name stored in cache
        cache match
        {
            // to be a parm-name used for
    ...
    private def genMap(args: List[String]) =
    {
        ...
        {
            argsMap
        } // must have pairs: -name value
        else
        {
            // to return a valid Map
            for (i <- 0.until(args.length, 2))
            {
                // iterate through args by 2
    ...
    private def validParms(args: Map[String, String]) =
    {
        ...
        for
        {(key, value) <- args // iterate through args
    ...
    def validate(args: List[String]) =
    {
        // validate args to parms
        ...
        if(reqList.isEmpty && validList.isEmpty)
        {
            // successful return
            (true, "", mergeParms(argsMap)) // true, "", mergedParms
        } else
          {
              (false, mkString(reqList, validList), Map[String, String]())
          }
    }
}